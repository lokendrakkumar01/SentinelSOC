...
import { Bell, ShieldAlert, CheckCircle, ExternalLink, Zap } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useSocket } from '../../context/SocketContext';
import api from '../../api/axios';
import SeverityBadge from '../common/SeverityBadge';
import { AttackSimulatorModal } from '../common/AttackSimulatorModal';
...
const Header: React.FC = () => {
  ...
  return (
    <>
      <header className="h-16 bg-slate-900 border-b border-slate-800 flex items-center justify-between px-6 z-20 relative">
        ...
        <div className="flex items-center space-x-4 md:space-x-6">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-cyan-950/60 hover:bg-cyan-900/80 border border-cyan-700/60 text-cyan-300 text-xs font-semibold shadow-[0_0_12px_rgba(6,182,212,0.25)] transition-all hover:shadow-[0_0_16px_rgba(6,182,212,0.4)]"
            title="Launch Interactive Attack Simulation Lab"
          >
      ...
      <AttackSimulatorModal isOpen={simulatorOpen} onClose={() => setSimulatorOpen(false)} />
    </>
  );
};
